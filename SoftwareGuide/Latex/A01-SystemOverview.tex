...
If ITK is installed, Python wrappers will be installed in the Python library
directory specified by the CMake option \code{PY\_SITE\_PACKAGES\_PATH}
(typically \code{lib/python2.7/site-packages}) inside the folder given by the
CMake option \code{CMAKE\_INSTALL\_PREFIX}. If a virtual environment is being used,
\code{CMAKE\_INSTALL\_PREFIX} can be modified to point to the folder corresponding
to that environment. If ITK is built with the CMake option \code{BUILD\_SHARED\_LIBS}
enabled, it will be necessary to add the installation directory containing the shared
objects to the correct environment variable (on Windows \code{PATH}; on Linux
\code{LD\_LIBRARY\_PATH}; on MacOS \code{DYLD\_LIBRARY\_PATH}).
